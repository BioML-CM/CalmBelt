...
def length_max_snps2(align_array,id_list,startstop_df,snps_max_df): #สำหรับหา position insertion
    ...
    #     print(sample_id)
        start_end_set = set()
        se_df = startstop_df[startstop_df.index ==sample_id].sort_values('length', ascending=False) #เรียงความยาวมากไปน้อย
...
#         print(se)
    ...
    #                 print(sample_id,min(new_pos),max(new_pos),j)
                ...
                snps_max_df = drop_data(snps_max_df,sample_id,intersec_set,j)  #ลบ snps ในเส้นที่สั้นกว่่า ในตำแหน่งที่ชนกัน
                start_end_set = start_end_set.union(set(range(a,b+1)))
...
def predict_clade(id_list,snps_max_df,align_array):
    clade_data = pd.read_excel('../preprocessed/clade.xlsx')
...
#     return check, r_p, s_p, int((p-gene_start-1)/3)+1
    ...
    if (sample_seq[x] == '-' and sample_seq[y] == '-' and sample_seq[z] == '-') or (sample_seq[x] != '-' and sample_seq[y] != '-' and sample_seq[z] != '-'):
...
def change_protein_table(change_df,position_gene_df,align_array_df,seq_ref_list):
    ...
    #     print(id_number)
        ...
        if gene != 'None':
    #         print(i,p, gene)
            check, ref_protein, sample_protein, pos_protein = check_protein(id_number, p,  gene, align_array_df, position_gene_df, seq_ref_list)
    #         print(check, ref_protein, sample_protein, pos_protein)
            change_protein += [[id_number, p, ref_protein, pos_protein, sample_protein, ref_protein+str(pos_protein)+sample_protein, gene, check]]
        gene_dict[p] = gene
        #             print(p,check)


    change_protein_df = pd.DataFrame(change_protein,columns=['id','pos_neucleotide', 'query_protein', 'position_protein', 'sbjct_protein', 'change_protein', 'gene', 'check'])
