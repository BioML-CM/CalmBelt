...
script_path = '/'.join(script_fpath.split('/')[:-1])

UPLOAD_DIRECTORY = os.path.join(script_path, "upload")
DOWNLOAD_DIRECTORY = os.path.join(script_path, "download")
...
def file_download_link(filename):
    ...
    location = urlquote(filename)
    return html.A(filename.split('/')[1], href=location)
...
count_pango_df = pd.read_csv(os.path.join(args.preprocess_dir, 'count_pango_df.csv'), index_col=0)
...
count_pango_country_df = pd.read_csv(os.path.join(args.preprocess_dir, 'count_pango_country_df.csv'), index_col=0)
count_pango_country_df['%'] = count_pango_country_df['count'] / count_pango_country_df['total'] * 100

country_lineage = {}

for r in [args.country_name]:
...
fig_clade_north_america = plot_data.fig_clade(os.path.join(args.preprocess_dir, f'North America_data_clade_df.csv'))
fig_clade_asia = plot_data.fig_clade(os.path.join(args.preprocess_dir, f'Asia_data_clade_df.csv'))
fig_clade_europe = plot_data.fig_clade(os.path.join(args.preprocess_dir, f'Europe_data_clade_df.csv'))
fig_clade_south_america = plot_data.fig_clade(os.path.join(args.preprocess_dir, f'South America_data_clade_df.csv'))
fig_clade_oceania = plot_data.fig_clade(os.path.join(args.preprocess_dir, f'Oceania_data_clade_df.csv'))
fig_clade_africa = plot_data.fig_clade(os.path.join(args.preprocess_dir, f'Africa_data_clade_df.csv'))

fig_who_north_america = plot_data.fig_who(os.path.join(args.preprocess_dir, f'North America_data_who_df.csv'))
fig_who_asia = plot_data.fig_who(os.path.join(args.preprocess_dir, f'Asia_data_who_df.csv'))
fig_who_europe = plot_data.fig_who(os.path.join(args.preprocess_dir, f'Europe_data_who_df.csv'))
fig_who_south_america = plot_data.fig_who(os.path.join(args.preprocess_dir, f'South America_data_who_df.csv'))
fig_who_oceania = plot_data.fig_who(os.path.join(args.preprocess_dir, f'Oceania_data_who_df.csv'))
fig_who_africa = plot_data.fig_who(os.path.join(args.preprocess_dir, f'Africa_data_who_df.csv'))

fig_sum_north_america = plot_data.fig_sum(os.path.join(args.preprocess_dir, f'North America_data_clade_df.csv'))
fig_sum_asia = plot_data.fig_sum(os.path.join(args.preprocess_dir, f'Asia_data_clade_df.csv'))
fig_sum_europe = plot_data.fig_sum(os.path.join(args.preprocess_dir, f'Europe_data_clade_df.csv'))
fig_sum_south_america = plot_data.fig_sum(os.path.join(args.preprocess_dir, f'South America_data_clade_df.csv'))
fig_sum_oceania = plot_data.fig_sum(os.path.join(args.preprocess_dir, f'Oceania_data_clade_df.csv'))
fig_sum_africa = plot_data.fig_sum(os.path.join(args.preprocess_dir, f'Africa_data_clade_df.csv'))

fig_clade_sin = plot_data.fig_clade(os.path.join(args.preprocess_dir, f'{args.country_name}_data_clade_df.csv'))
fig_sum_sin = plot_data.fig_sum(os.path.join(args.preprocess_dir, f'{args.country_name}_data_clade_df.csv'))
fig_who_sin = plot_data.fig_who(os.path.join(args.preprocess_dir, f'{args.country_name}_data_who_df.csv'))

fig_enp_sin = plot_data.fig_enp(args.preprocess_dir)
fig_cluster_k_mean_sin = plot_data.fig_cluster(os.path.join(args.preprocess_dir, f'Y_tse.pickle'), 'k_mean')
fig_cluster_who_sin = plot_data.fig_cluster(os.path.join(args.preprocess_dir, f'Y_tse.pickle'), 'who_name')
...
fig_dendro_sin, _, _ = plot_data.fig_dendro(args.preprocess_dir, 'who_name')
...
about_layout = html.Div([
    ...
    html.P(['''The current pandemic caused by SARS-CoV-2 virus raises great concern around the world. 
            ...
            html.A(children='a standard reference genome.', href='https://www.ncbi.nlm.nih.gov/sars-cov-2/',
                   target='_blank'),
            ...
            html.A(children='A recent study', href='https://www.nature.com/articles/d41586-020-02544-6',
                   target='_blank'),
    ...
    html.P([html.A(children='SARS-CoV-2  GISAID Clades', href='https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5388101/',
                   target='_blank')]),
    html.Br(),
    ...
    html.Br(),
    html.P([html.A(children='Pango Lineages', href='https://www.who.int/en/activities/tracking-SARS-CoV-2-variants/',
                   target='_blank', )])
    , html.Br(),
    html.P(html.Table([
...
region_layout = html.Div([
    ...
    html.P([
               '''Overall statistics of COVID-19 cases in Asia. A time-series plot shows the percentage and number of cases for each clade or lineage (7-day average)''']),
    ...
    html.P([
               '''Overall statistics of COVID-19 cases in Europe. A time-series plot shows the percentage and number of cases for each clade or lineage (7-day average)''']),
    ...
    html.P([
               '''Overall statistics of COVID-19 cases in North America. A time-series plot shows the percentage and number of cases for each clade or lineage (7-day average)''']),
    ...
    html.P([
               '''Overall statistics of COVID-19 cases in South America. A time-series plot shows the percentage and number of cases for each clade or lineage (7-day average)''']),
    ...
    html.P([
               '''Overall statistics of COVID-19 cases in Africa. A time-series plot shows the percentage and number of cases for each clade or lineage (7-day average)''']),
    ...
    html.P([
               '''Overall statistics of COVID-19 cases in Oceania. A time-series plot shows the percentage and number of cases for each clade or lineage (7-day average)''']),
...
# # For a selected country (args.country_name)

# In[18]:


sin_layout = html.Div([
    html.H3(f'{args.country_name} trends (COVID-19)'),
    html.P([
               f'''Overall statistics of COVID-19 cases in {args.country_name}. A time-series plot shows the percentage and number of cases for each clade or lineage (7-day average)''']),
    html.Div(children=[
        dcc.Loading(
            id="loading-plot_sin",
            type="default",
    ...
    html.P(['''Top 5 PANGO lineages in last 30 days = ''', ', '.join(country_lineage[args.country_name])]),
    html.Hr(),
    ...
    html.P([
               '''Multiple SARS-CoV-2 genomes are clustered based on top 50 positions with highest diversity and mutations predefined in GISAID. Each dot represents a genome and colors represent different lineages (Left) and k-mean clustering results (Right). ''',
               ]),

    html.Div([
        html.Div([
            html.Img(src='data:image/png;base64,{}'.format(call_image('download/fig_cluster_who_sin.png')),
                     style={'width': '86%'})
            #                                 dcc.Graph(id='g1', figure=fig_cluster_result_thai,)
        ], className="six columns"),
        html.Div([
            html.Img(src='data:image/png;base64,{}'.format(call_image('download/fig_cluster_k_mean_sin.png')),
                     style={'width': '86%'})
...
build_layout = html.Div([
    ...
    html.A(children='S15_03-03-2020_C.fasta', href="download/{}".format(urlquote('S15_03-03-2020_C.fasta'))),
    html.Br(),
    html.A(children='S16_04-04-2020_D.fasta', href="download/{}".format(urlquote('S16_04-04-2020_D.fasta'))),
    html.Br(),
    html.A(children='S21_17-03-2020_H.fasta', href="download/{}".format(urlquote('S21_17-03-2020_H.fasta'))),
    html.Br(),
    html.A(children='S28_13-03-2020_H.fasta', href="download/{}".format(urlquote('S28_13-03-2020_H.fasta'))),
    html.Br(),
    html.A(children='S29_14-03-2020_C.fasta', href="download/{}".format(urlquote('S29_14-03-2020_C.fasta'))),
    html.Br(),
...
def update_output(uploaded_filenames, uploaded_file_contents):
    ...
    sample_id_list = [n.split('_')[0] for n in uploaded_filenames]
    if not (len(set(sample_id_list)) == len(sample_id_list)):
        return html.Div([html.P(['''Found duplicated samples IDs! Please make sure that your sample IDs are unique.'''])])

    if uploaded_filenames is not None and uploaded_file_contents is not None:
        # copy file reference + align
        t = time.localtime()
        timestamp = time.strftime('%b-%d-%Y_%H%M%S', t)
        path = (f'{UPLOAD_DIRECTORY}/' + timestamp + '/')
        os.makedirs(path)

        ref_name = 'Reference-30-12-2019_30-12-2019_Reference.fasta'
        original = args.reference_fname
        target = (path + ref_name)
        shutil.copyfile(original, target)

        lin_sample, clade_test, change_protein, align_array, snps_max, len_genome, snps_insertion, choose_pos, start_end = align_array_test(
            ref_name, path)
        ...
        if len(list_file) == 0:
            ...
            fig_dendro_built, fig_heatmap, result_upload_df, download_NAME, group_dict = prepare_plot_dendro(X_df, clade_dendro_df,
                                                                                                 change_protein_df,
                                                                                                 snps_max_df,
                                                                                                 start_stop_df,
                                                                                                 insertion_sum_df)

            built.built_iqtree(group_dict, path)
            built.built_timetree(group_dict, path)
            ...
            return html.Div([
                ...
                html.Div([
                    html.Div([
                        html.P(['''Phylogenetic tree (IQTREE2 used in Pangolin)''']),
                        html.Img(src='data:image/png;base64,{}'.format(call_image(os.path.join(path, 'iqtree.png'))),
                                 style={'width': '90%'})
                        #                                 dcc.Graph(id='g1', figure=fig_cluster_result_thai,)
                    ], className="six columns"),
                    html.Div([
                        html.P(['''Time stamped phylogenetic tree (TimeTree used in Nextstrain)''']),
                        html.Img(src='data:image/png;base64,{}'.format(call_image(os.path.join(path, 'timetree.png'))),
                                 style={'width': '90%'})
                        #                                 dcc.Graph(id='g2', figure=fig_cluster_k_mean_thai)
                    ], className="six columns"),
                ], className="row"),
...
def count_lin(n_clicks, input1, input2, input3, input4):
    if input1 != None and input2 != None:
        ...
        count_metadata_df = pd.read_csv(os.path.join(args.preprocess_dir, 'count_metadata_df.csv'))
        ...
        month_count_df = month_count_df[['country_1', 'percentage', 'count', 'sum', 'lin']].sort_values('percentage',
...
def insertion_summary(snps_insertion_df, choose_pos_df):
    ...
    insertion_sum_df = insertion_sum_df[(insertion_sum_df['position_ref'] >= insertion_sum_df['start_new']) & (
                insertion_sum_df['position_ref'] <= insertion_sum_df['end_new'])]
...
def prepare_plot_dendro(X_df, clade_dendro_df, change_protein_df, snps_max_df, start_stop_df, insertion_sum_df):
    ...
    result_upload_df.columns = ['Id', 'Lineage', 'Clade', 'SNPs', 'Deletion', 'Insertion', 'Amino acid', 'Frame shift',
                                'Coverage']
    ...
    fig, fig_heatmap, position_dendro_df, group_dict, heatmap_df = built.fig_dendro_built_iqtree(X, X_id_list,clade_dendro_df,snps_max_df,change_protein_df)
    
    heatmap_df.to_excel(writer, sheet_name='Sheet2')
    writer.save()
    
    return fig, fig_heatmap, result_upload_df, download_NAME, group_dict
...
gene_df = pd.read_csv('gene.tsv', sep='\t')
...
alarm_layout = html.Div([
    html.Div(className='row',  # Define the row element
             children=[
                 ...
                 html.P([
                            f'''Statistics are based on 3,406 genomes in {args.country_name} (downloaded from GISAID as of 30 June 2021).''']),
    ...
    html.P(['''Nucleotide position start ''',
            dcc.Input(id="input1", type="text", value=23400, size='8',
    ...
    html.P(['''Amino acid position start ''',
            dcc.Input(id="input1-aa", type="text", value=500, size='8',
                      style={"margin-left": "10px", "margin-right": "10px"}),
            ''' end ''',
    ...
    html.P(['''Upload Excel file containing a list of amino acid changes ''',
            html.A(children='(example file)', href="download/{}".format(urlquote('test_amino_acid_change.xlsx'))),
            ''', where the first columns is a protein name (i.e. S, E, N) and the second columns contains amino acid change (i.e. D614G, N501Y)''']),
    dcc.Upload(
...
def plot_alarm_neu(n_clicks, input1, input2):
    if input1 != None and input2 != None:
        ...
        neucleotide_alarm_df = pd.read_csv(os.path.join(args.preprocess_dir, 'neucleotide_alarm_df.csv'))
        del_alarm_df = pd.read_csv(os.path.join(args.preprocess_dir, 'del_alarm_df.csv'))
        insert_alarm_df = pd.read_csv(os.path.join(args.preprocess_dir, 'insert_alarm_df.csv'))
        return html.Div([
            html.H3('''SNPs'''),
...
def plot_alarm_aa(n_clicks, input1, input2, protein):
    if input1 != None and input2 != None:
        ...
        max_range = int(input2)
        position = range(min_range, max_range + 1)
        protein_alarm = pickle.load(open(os.path.join(args.preprocess_dir, 'protein_alarm.pickle'), "rb"))
...
def update_output(uploaded_filenames, uploaded_file_contents):
    if uploaded_filenames is not None and uploaded_file_contents is not None:
        ...
        if '.xls' in uploaded_filenames:
            ...
            protein_alarm_pickle_fname = os.path.join(args.preprocess_dir, 'protein_alarm.pickle')
            return dcc.Graph(figure=plot_data.alarm_excel_plot(gene_list, pos_list, protein_alarm_pickle_fname))
        else:
            return html.Div([
...
def plot_alarm_specific_protein(n_clicks, input1, input2, input3, input4, input5, input6, input7, input8, input9,
                                input10, input11, input12, input13, input14, input15, input16, input17, input18,
                                input19, input20, input21, input22, input23, input24, input25):
    gene_list = ['NSP1', 'NSP2', 'NSP3', 'NSP4', 'NSP5', 'NSP6', 'NSP7', 'NSP8', 'NSP9', 'NSP10', 'NSP12', 'NSP13',
                 'NSP14', 'NSP15', 'NSP16', 'S', 'ORF3a', 'E', 'M', 'ORF6', 'ORF7a', 'ORF7b', 'ORF8', 'N', 'ORF10']
    position_list = [input1, input2, input3, input4, input5, input6, input7, input8, input9, input10, input11, input12,
                     input13, input14, input15, input16, input17, input18, input19, input20, input21, input22, input23,
                     input24, input25]
    position_and_gene = []
    ...
    if (gene_list != []) and (pos_list != []):
        return dcc.Graph(figure=plot_data.alarm_specific_protein(gene_list, pos_list, args.preprocess_dir))
    else:
        html.Div([html.H1(''),
                  ])
    ...
    html.Div(className='row',  # Define the row element
             children=[
                 html.Div(className='two columns div-user-controls',
                          children=[
                              html.Img(src='data:image/png;base64,{}'.format(call_image('download/logo_02.png')),
                                       style={'width': '100%'}),
                              html.Br(), html.Br(),
...
def display_page(pathname):
    tabs = [
        dcc.Tabs(
            ...
            children=[
                ...
                dcc.Tab(label=f'{args.country_name} Stats', value=f'/{args.country_name}'),
                dcc.Tab(label='Analyse Genomes', value='/analyse'),
                dcc.Tab(label=f'Inspect ROI ({args.country_name})', value='/inspect'),
            ], style={
                'width': '100%',
                'height': '50%'
            },
    ...
    elif pathname == f'/{args.country_name}':
...
def align_array_test(file_name, upload_path):
    ...
    command_pango_list = ['pangolin', '--outdir', f'{upload_path}', '--outfile ', f'{upload_path}{Pango_outfile}',
    ...
    command_list = ['blastn', '-query', ref_path, '-subject', f'{upload_path}{file_name}', '-outfmt', '5', '-out',
                    f'{upload_path}{BACKUP_NAME}']
    ...
    sum_length, snps_max_df, align_array, choose_pos_list = utils.length_max_snps2(align_array, id_list, start_end_df,
                                                                                   snps_max_df)  # หาตำแหน่งของ insertion
    ...
    # predict clade
    clade_test_df = utils.predict_clade(id_list, snps_max_df, align_array, args.preprocess_dir)
    clade_test = clade_test_df['result']

    # check protein
    position_gene_df = pd.read_csv(os.path.join(args.preprocess_dir, 'position_gene_df.csv'))
    ...
    change_protein_df['change_protein'] = [str(q) + str(p) + str(s) for q, p, s in
                                           zip(change_protein_df['query_protein'],
                                               change_protein_df['position_protein'],
                                               change_protein_df['sbjct_protein'])]
...
if __name__ == '__main__':
    #     app.run_server(debug=True, port=8889, use_reloader=False)
