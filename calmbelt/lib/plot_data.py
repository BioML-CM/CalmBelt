...
def fig_clade(df_fname):
    data_df = pd.read_csv(df_fname, index_col=0)
...
def fig_enp(preprocess_dir):
    count_align_df = pd.read_csv(os.path.join(preprocess_dir, 'count_align_df.csv'))
...
def fig_cluster(y_tse_fname, cluster_type):
    Y_tse = pickle.load(open(y_tse_fname, "rb"))
    Y_tse = Y_tse.sort_values([cluster_type])

    color_list = ['cornflowerblue', 'darkorange', 'deepskyblue', 'pink', 'plum', 'lightcoral', 'goldenrod',
    ...
    if cluster_type == 'k_mean':
        ...
        cluster_type = 'WHO'
        color_list = ['cornflowerblue', 'darkorange', 'pink', 'plum', 'deepskyblue', 'lightcoral', 'goldenrod',
                      'lightseagreen', 'mediumseagreen']

    clade = Y_tse['{}'.format(cluster_type)].values

    symbol_list = ['circle', 'cross', 'x', 'triangle-up', 'triangle-down', 'square', 'diamond']
    symbol_open = ['circle-open', 'cross-open', 'x-open', 'triangle-up-open', 'triangle-down-open', 'square-open',
                   'diamond-open']
...
def fig_dendro(preprocess_dir, color_style):
    X_dendro = pickle.load(open(os.path.join(preprocess_dir, 'X_dendro.pickle'), "rb"))
    X_id_list = pickle.load(open(os.path.join(preprocess_dir, 'X_id_list_dendro.pickle'), "rb"))
    change_protein_df = pd.read_csv(os.path.join(preprocess_dir, 'summary_change_protein_df.csv'))
    snps_max_df = pd.read_csv(os.path.join(preprocess_dir, 'snps_max_df.csv')).sort_values('id')
    clade_drop_dendro = pickle.load(open(os.path.join(preprocess_dir, 'clade_dendro.pickle'), "rb"))
    ...
    position_dendro_df = pd.concat(
        [position_dendro_df, clade_drop_dendro[['date', 'year', 'month', 'day', color_style]]], axis=1)
    ...
    for i in range(0, len(labels_list)):
        ...
        fig_dendro.add_trace(go.Scatter(x=[0, xnew], y=[5 + i * 10, 5 + i * 10], mode='lines', hoverinfo='none',
                                        opacity=0.1,
                                        showlegend=False,
    ...
    # if sample_type == 'asia':
    #     fig_dendro.update_yaxes(showticklabels=False, range=(
    #     position_dendro_df['ynew'].min() - 1200, position_dendro_df['ynew'].max() + 1200))
    #     fig_dendro.update_xaxes(range=(position_dendro_df['xnew'].min() - 2, position_dendro_df['xnew'].max() + 0.3))
    # else:
    fig_dendro.update_yaxes(showticklabels=False,
                            range=(position_dendro_df['ynew'].min() - 300, position_dendro_df['ynew'].max() + 300))
    fig_dendro.update_xaxes(range=(position_dendro_df['xnew'].min() - 2, position_dendro_df['xnew'].max() + 0.3))
...
def alarm_excel_plot(gene_list, pos_list, protein_alarm_pickle_fname):
    df = pickle.load(open(protein_alarm_pickle_fname, "rb"))
...
def alarm_specific_protein(gene_list, pos_list, preprocess_dir):
    # load data
    id_month = pickle.load(open(os.path.join(preprocess_dir, "id_month.pickle"), "rb"))
    id_month_key = sorted(list(id_month.keys()))

    summary_change_protein_df = pd.read_csv(os.path.join(preprocess_dir, 'summary_change_protein_df.csv'))
